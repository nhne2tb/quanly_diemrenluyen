...
// ================= Lấy dữ liệu =================
$khoas = $conn->query("SELECT * FROM tb_khoa ORDER BY ngay_tao DESC")->fetchAll(PDO::FETCH_ASSOC);
$total = count($khoas);
?>
<!doctype html>
<html lang="vi">
...
.card-title{color:var(--brand);font-weight:700;font-size:1.25rem;white-space:nowrap;}
.card-header .input-group{flex:1;min-width:200px;}
.btn-brand{background:var(--brand);color:#fff;border-radius:.5rem;white-space:nowrap;}
.btn-brand:hover{background:var(--brand-hover);color:#fff;}

/* ============= BẢNG ============= */
.table{border-collapse:separate;border-spacing:0 .5rem;margin:0;}
.table thead th{
...
/* icon mũi tên cho Chi tiết */
.action-btn.detail i {
  font-size: .9rem;
  transition: transform .2s;
}
.action-btn.detail[aria-expanded="true"] i {
...
.details-content strong{min-width:90px;display:inline-block;color:var(--muted);}
</style>
</head>
<body>
<div class="wrap">
  ...
  <div class="main-card">
    ...
    <div class="table-responsive">
      <table class="table align-middle" id="khoaTable">
        <thead>
          <tr>
            <th style="width:100px;">Mã</th>
            <th style="min-width:300px;">Tên khoa</th>
            <th style="min-width:220px;">Trưởng khoa</th>
        ...
        <?php if(empty($khoas)): ?>
          <tr>
            <td colspan="4" class="text-center text-muted py-5">
              <i class="bi bi-diagram-3 fs-2"></i><br>Chưa có dữ liệu khoa.
            </td>
          </tr>
        <?php else: foreach($khoas as $i=>$k): ?>
          <tr class="data-row">
            <td><span class="badge-code"><?= h($k['ma_khoa']) ?></span></td>
            <td>
              <div class="fw-semibold"><?= h($k['ten_khoa']) ?></div>
              <small class="text-muted">Tạo ngày <?= date('d/m/Y H:i', strtotime($k['ngay_tao'])) ?></small>
            </td>
            <td><?= h($k['truong_khoa']) ?: '—' ?></td>
            <td>
          ...
          <tr class="collapse details-row" id="details<?= $i ?>">
            <td colspan="4">
              <div class="details-content row">
                <div class="col-md-6 item"><strong>Phó khoa:</strong> <?= h($k['pho_khoa']) ?: '—' ?></div>
                <div class="col-md-6 item"><strong>Email:</strong> <?= $k['email'] ? '<a href="mailto:'.h($k['email']).'">'.h($k['email']).'</a>' : '—' ?></div>
                <div class="col-md-6 item"><strong>SĐT:</strong> <?= h($k['sdt']) ?: '—' ?></div>
                <div class="col-md-6 item"><strong>Website:</strong> <?= h($k['website']) ?: '—' ?></div>
                <div class="col-12 item"><strong>Địa chỉ:</strong> <?= h($k['dia_chi']) ?: '—' ?></div>
              </div>
            </td>
          </tr>
...
document.getElementById('tableSearch').addEventListener('input',function(e){
  ...
  document.querySelectorAll('#khoaTable tbody tr.data-row').forEach(r=>{
